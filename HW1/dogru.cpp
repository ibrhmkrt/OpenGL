...
void lineSegment(void)
{
	...
	glColor3f(1.0, 0.0, 0.0);//Çizginin rengini kýrmýzý yapar
	glLineWidth(3.0);//Çizgi kalýnlýðýný 3 yaptýk               
	glBegin(GL_LINES);//Çizeceðimiz þekili Çizgi olarak belirttik
	glVertex2f(-0.9, 0.9);//1.Nokta
	glVertex2f(0.9, -0.9);//2. Nokta 1-2 noktalarý çizgi oluþturur
	glVertex2f(0.9, 0.9);//3.Nokta
	glVertex2f(-0.9, -0.9);//4. Nokta 3-4 noktalarý çizgi oluþturur
...
int main(int argc, char* argv[])
{
	...
	glutCreateWindow("soru1");	// Pencerenin ismini yazdýk ve pencere oluþturduk
